...
async def today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_day >= TOTAL_DAYS:
        await update.message.reply_text("🎉 Поздравляю! Ты прошёл все 30 дней!")
        return
...
async def repeat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if last_day == 0:
        await update.message.reply_text("Ты ещё не получил ни одного дня. Используй /today.")
        return
